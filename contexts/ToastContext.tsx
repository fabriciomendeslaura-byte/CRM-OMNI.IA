    ...
    <ToastContext.Provider value={{ addToast, removeToast }}>
      ...
      <div className="fixed bottom-4 right-4 z-50 flex flex-col gap-2">
        {toasts.map((toast) => (
          <div
            key={toast.id}
            className={`flex items-start gap-3 p-4 rounded-lg shadow-lg border w-80 transition-all transform translate-x-0 ${
              toast.type === 'success' ? 'bg-surface border-green-900 text-green-100' :
